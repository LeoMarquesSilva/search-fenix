...
export function resolveExportDatasets(
  ...
  const set = new Set<CompanyDataset>();
  if (uiSelected.length > 0) {
    ...
    for (const raw of fromApi) {
      if (EXPORT_DATASET_ORDER.includes(raw as CompanyDataset)) {
        set.add(raw as CompanyDataset);
      }
    }
  }
...
function buildCompleteSheet(company: CompanyDto): (string | number)[][] {
  ...
  if (company.atividades_secundarias?.length) {
    ...
    for (const a of company.atividades_secundarias) {
      aoa.push([
        a.classe != null && a.classe !== "" ? asExcelText(a.classe) : "",
        a.desc_classe ?? "",
        a.desc_subclasse ?? "",
        a.divisao != null && a.divisao !== "" ? asExcelText(a.divisao) : "",
        a.grupo != null && a.grupo !== "" ? asExcelText(a.grupo) : "",
        a.secao ?? "",
        a.subclasse != null && a.subclasse !== "" ? asExcelText(a.subclasse) : "",
        a.ramo_de_atividade ?? "",
        a.segmento ?? "",
      ]);
    }
